...
TYPE_KEYWORDS = {
    ...
    "risk": ("风险",),
    "forecast": ("预测", "预估", "forecast"),
}
TIME_KEYWORDS = ("date", "time", "month", "日期", "时间", "月份", "月")
METRIC_KEYWORDS = ("revenue", "sales", "amount", "营收", "收入", "销售", "金额")
...
def session_title(objective: str) -> str:
    ...
    metric = "营收" if any(term in text for term in ("营收", "收入", "revenue")) else "销售" if any(term in text for term in ("销售", "sales")) else ""
...
def plan_question(frame: pd.DataFrame, objective: str) -> QuestionPlan:
    ...
    metric_column = _match([str(column) for column in frame.select_dtypes(include="number").columns], METRIC_KEYWORDS)
    if metric_column is None:
        numeric = frame.select_dtypes(include="number").columns.tolist()
        metric_column = str(numeric[0]) if numeric else None
