...
type Risk = { title: string; object?: string; level: string; evidence: string[]; human_review_required?: boolean; mitigation: string };
...
type TaskTabSession = { id: string; title: string; objective: string };
export function TaskTabs({ sessions, recentSessions = sessions, activeId, onSelect, onClose, onNew }: { sessions: TaskTabSession[]; recentSessions?: TaskTabSession[]; activeId: string | null; onSelect: (id: string) => void; onClose: (id: string) => void; onNew: () => void }) {
  ...
  return <div className="task-tabs-wrap"><div className="session-tabs" role="tablist" aria-label="已打开的分析任务" onWheel={(event) => { event.currentTarget.scrollLeft += event.deltaY; }}>{sessions.map((session) => <button role="tab" aria-selected={activeId === session.id} aria-label={`${session.title}：${session.objective}`} title={session.objective} className={activeId === session.id ? "active" : ""} style={{ minWidth: 120, maxWidth: 220 }} key={session.id} onClick={() => onSelect(session.id)}><span className="tab-title" style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{session.title}</span><span role="button" tabIndex={0} aria-label={`关闭 ${session.title}`} onClick={(event) => { event.stopPropagation(); onClose(session.id); }}>×</span></button>)}<button aria-label="新建分析标签" onClick={onNew}>＋</button></div><button className="all-tasks" aria-label="全部任务" onClick={() => setShowAll((value) => !value)}>全部任务</button>{showAll && <div className="all-tasks-menu" role="menu">{recentSessions.map((session) => <button key={session.id} onClick={() => { onSelect(session.id); setShowAll(false); }}>{session.title}</button>)}</div>}</div>;
}
...
export function ResultPanel({ session, tab }: { session: Session | null; tab: ResultTab }) {
  ...
  if (tab === "结果") return <div className="result-content"><h2>核心结论</h2><p className="core-conclusion">{session.core_conclusion}</p><h3>关键数据</h3><div className="kpi-grid">{session.key_metrics?.map((metric) => <Kpi key={metric.label} label={metric.label} value={metric.value} detail={metric.detail} />)}</div><h3>详细分析</h3>{session.sections?.map((section) => <section className="analysis-section" key={section.title}><h4>{section.title}</h4><ul>{section.items.map((item, index) => <li key={index}>{item.text}</li>)}</ul></section>)}<h3>业务风险</h3>{session.business_risks?.map((risk) => <article className="risk-card" key={risk.title}><div><span className={`risk-level ${risk.level}`}>{risk.level}</span><strong>{risk.title}</strong></div><p>{risk.evidence.join(" ")}</p><small>风险原因：{risk.mitigation}</small></article>)}<h3>建议</h3>{session.suggestions?.map((option) => <article className="option-card" key={option.name}><strong>{option.name}</strong><p>{option.expected_benefit}</p><small>下一步：{option.next_step}</small></article>)}<details className="data-quality"><summary>数据质量与分析限制</summary><p>{session.data_quality?.summary}</p><ul>{[...(session.data_quality?.limitations ?? []), ...(session.limitations ?? [])].map((item, index) => <li key={index}>{item}</li>)}</ul></details></div>;
