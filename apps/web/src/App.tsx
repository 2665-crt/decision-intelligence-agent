...
function SettingsDialog({ providers, onClose, onError, onSaved }: { providers: Provider[]; onClose: () => void; onError: (value: string) => void; onSaved: () => Promise<void> }) {
  ...
  return <div className="dialog-backdrop"><form className="create-dialog" onSubmit={save}><div><p className="pane-label">Model / API</p><h2>模型设置</h2><p>Key 仅保存到本机 .env，保存后不会回显。</p></div><label>Provider<select value={provider} onChange={(event) => chooseProvider(event.target.value)}>{providers.filter((item) => item.id !== "simulated").map((item) => <option key={item.id} value={item.id}>{item.display_name}</option>)}</select></label><p className="settings-status">{selected?.configured ? `已配置：${selected.api_key_masked}` : "尚未配置 Key"}</p><label>API Key<input type="password" value={apiKey} onChange={(event) => setApiKey(event.target.value)} placeholder="留空则保留已保存的 Key" /></label><label>Base URL<input value={baseUrl} onChange={(event) => setBaseUrl(event.target.value)} /></label><label>模型名<input value={model} onChange={(event) => setModel(event.target.value)} /></label>{notice && <p className="settings-status">{notice}</p>}<div className="dialog-actions"><button type="button" className="ghost" onClick={onClose}>取消</button><button type="button" className="ghost" onClick={() => void testConnection()}>测试连接</button><button type="submit">保存</button></div></form></div>;
}
